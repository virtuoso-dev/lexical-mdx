...
export type SerializedPetyoNode = Spread<
  {
...
export class PetyoNode extends DecoratorBlockNode {
  __contents: LexicalEditor;

  constructor(
    contents?: LexicalEditor,
  ...
  static clone(node: PetyoNode): PetyoNode {
    return new PetyoNode(node.__id, node.__format, node.__key);
  }

  static importJSON(serializedNode: SerializedPetyoNode): PetyoNode {
    const node = new PetyoNode();
  ...
  exportJSON(): SerializedPetyoNode {
    return {
      ...super.exportJSON(),
...
export function $createPetyoNode(): PetyoNode {
  return new PetyoNode();
}

export function $isPetyoNode(
  node: PetyoNode | LexicalNode | null | undefined
): node is PetyoNode {
  return node instanceof PetyoNode;
}
